...
{
    public function create(Csv $csvProcessor): Reader
    {
        try {
            return Reader::createFromPath($csvProcessor->csvFile->getPathname())
                ->setHeaderOffset(0)
                ->setDelimiter($csvProcessor->delimiter)
                ->setEnclosure($csvProcessor->enclosure)
                ->setEscape($csvProcessor->escape);
